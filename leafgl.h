...
int glMainRenderLoop(SDL_Renderer* renderer, SDL_Window* window, Polygon3d** models, float x_pos, float y_pos, float z_pos, char rotation_axis, float rotation_angle, int num_models, bool beauty_turn);
...
void gldrawScene(SDL_Renderer* renderer, Polygon3d** models, int num_models, char rotation_axis, float rotation_angle, float x_pos, float y_pos, float z_pos) {
    ...
    for (int model_idx = 0; model_idx < num_models; model_idx++) {
        ...
        for (int i = 0; i < current_model->num_faces; i++) {
            ...
            // normalize
            rotation_axis = tolower(rotation_axis);
            ...
            if (rotation_axis == 'y') {
                ...
                v1 = rotateX(current_model->vertices[face.v_idx[0]], rotation_angle);
                v2 = rotateX(current_model->vertices[face.v_idx[1]], rotation_angle);
                v3 = rotateX(current_model->vertices[face.v_idx[2]], rotation_angle);
            }
...
// legacy function
// int glRenderPolygon3D(SDL_Renderer* renderer, SDL_Window* window, Polygon3d* model, SDL_Event e ,const char* model_file_name, float polygon_angle_3D, float x_pos, float y_pos, float z_pos, int num_models);
